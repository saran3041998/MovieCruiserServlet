...
<body>
	<header class="header container-fluid">
		...
		<img src="images/movie-crusier.png" class="center"> <a
			class="header-itemright" href="ShowMovieListAdminServlet"
			style="margin: 15px">Movies</a>
	</header>
		...
		<table style="width: 100%;" id="books">
			...
			<c:forEach var="item" items="${adminmovieList}">
				<tr>
					...
					<td class="td-price"><fmt:formatNumber type="currency"
							value="${item.box_office }" /></td>
					<td>${item.active ? 'Yes' : 'No'  }</td>
					<td><fmt:formatDate pattern="dd/MM/yyyy"
							value="${item.dateOfLaunch }" /></td>
					<td>${item.genre}</td>
					<td>${item.hasTeaser ? 'yes' : 'No' }</td>
					<td><a href="ShowEditMovieServlet?id=${item.id}">Edit</a></td>
				</tr>
			</c:forEach>
